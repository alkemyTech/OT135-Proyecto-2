...
def delay(CreationDates, LastEditDates):
    ...
    for date in LastEditDates:
        delayTimes.append(
            (date - CreationDates[LastEditDates.index(date)]).total_seconds())
    return delayTimes
...
def query3(myroot):
    ...
    Aplica técnicas de map-reduce y devuelve el tiempo promedio de entre la fecha de creación y la fecha de ultima modificación de un post en días.
    '''
